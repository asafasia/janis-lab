...
amplitudes = np.linspace(a_min, a_max, Na)
...
if simulate:
    ...
    plt.pcolor((qubit_freq - qubit_LO + frequencies) / u.MHz, amplitudes * saturation_amp * 1e3, y.T)
    plt.ylabel("drive amplitude [mV]")
    plt.xlabel("drive freq [MHz]")
    plt.colorbar()
    plt.tight_layout()
    plt.axvline(x=0, color='r', linestyle='--', label='Qubit frequency')
    # plt.axvline(x=(qubit_freq - qubit_LO) / u.MHz, color='r', linestyle='--', label='Qubit LO')
    # plt.xlim([(qubit_LO - frequencies)[0] / u.MHz, (qubit_LO - frequencies)[-1] / u.MHz])
    plt.legend()
    plt.show()
    # for y0 in y.T:
    #     plt.plot((qubit_freq - qubit_LO + frequencies) / u.MHz, y)
    # plt.xlabel("drive freq [MHz]")
