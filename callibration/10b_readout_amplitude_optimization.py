...
a_min = 0
a_max = 1
amplitudes = np.linspace(0.5, 2, N)  # The amplitude vector +da/2 to add a_max to the scan
...
if simulate:
    ...
    fidelity_vec = []
    ground_fidelity_vec = []
    excited_fidelity_vec = []
    for i in range(len(amplitudes)):
        angle, threshold, fidelity, gg, ge, eg, ee = two_state_discriminator(
            I_g[i], Q_g[i], I_e[i], Q_e[i], b_print=True, b_plot=False
        )
        fidelity_vec.append(fidelity)
        ground_fidelity_vec.append(gg)
        excited_fidelity_vec.append(ee)

    ground_fidelity_vec = np.array(ground_fidelity_vec)
    excited_fidelity_vec = np.array(excited_fidelity_vec)

    # Plot the data
    plt.figure()
    plt.plot(amplitudes * readout_amp, fidelity_vec, "b.-", label="averaged fidelity")
    plt.plot(amplitudes * readout_amp, ground_fidelity_vec*100, "r.-", label="ground fidelity")
    plt.plot(amplitudes * readout_amp, excited_fidelity_vec*100, "r.-", label="excited fidelity")
