...
# OPX config
cg = config_generator.ConfigGenerator(output_offsets={I_channel: I_offset, Q_channel: Q_offset},
                                      input_offsets={1: 0.0, 2: 0.0})
...
g = g_Q / g_I
eps = 0.2
ret = optimize.minimize(check_with_model_corr, x0=[g, phi], method="Nelder-Mead",
                        args=(qm, "mixer", sa, lo_freq, if_freq),
                        options={"xatol": 1e-4, "fatol": 2,
                                 "initial_simplex": np.array(
                                     [[np.max([g - eps, 0]), phi + eps], [np.min([g + eps, 1]), phi + eps],
                                      [g, phi - eps]])})
...
g_I_cal = ret.x[0] * g_Q
phi_cal = ret.x[1]
model_cal = np.sqrt(g_I_cal ** 2 * np.cos(theta) ** 2 + g_Q_cal ** 2 * np.sin(theta) ** 2 + g_I_cal * g_Q_cal * np.sin(
    phi_cal) * np.sin(2 * theta))
plt.figure(1)
