...
if simulate:
    ...
    plt.subplot(221)
    plt.cla()
    plt.pcolor(amplitudes * x180_amp/1e-3, nb_of_pulses, I)
    plt.xlabel("Rabi pulse amplitude [mV]")
    plt.ylabel("# of Rabi pulses")
    plt.title("I quadrature [V]")
    plt.subplot(222)
    plt.cla()
    plt.pcolor(amplitudes * x180_amp/1e-3, nb_of_pulses, Q)
    plt.xlabel("Rabi pulse amplitude [mV]")
    plt.title("Q quadrature [V]")
    plt.subplot(212)
    plt.cla()
    plt.plot(amplitudes * x180_amp/1e-3, np.sum(I, axis=0))
    plt.xlabel("Rabi pulse amplitude [mV]")
    plt.ylabel("Sum along the # of Rabi pulses")
    plt.pause(0.1)
    plt.tight_layout()
    print(f"Optimal x180_amp = {amplitudes[np.argmin(np.sum(I, axis=0))] * x180_amp:.12f} V")
