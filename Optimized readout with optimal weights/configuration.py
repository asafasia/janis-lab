...
# Qubits
qubit_IF = 4050000000.0 - 3915318336
qubit_ef_IF = 4050000000.0 - 3915318336
# anaharmonicity 12-01 = -163.320497966974e6
qubit_LO = 4050000000.0
qubit_ef_LO = 4050000000.0
...
x180_wf, x180_der_wf = np.array(
    drag_gaussian_pulse_waveforms(x180_amp, x180_len, x180_sigma,anharmonicity=200.01e6, alpha=drag_coef, detuning=det)
)
...
x90_wf, x90_der_wf = np.array(
    drag_gaussian_pulse_waveforms(x90_amp, x90_len, x90_sigma,anharmonicity=200.01e6, alpha=drag_coef,  detuning=det)
)
...
minus_x90_wf, minus_x90_der_wf = np.array(
    drag_gaussian_pulse_waveforms(
        minus_x90_amp, minus_x90_len, minus_x90_sigma,anharmonicity=200.01e6, alpha=drag_coef, detuning=det
    )
)
...
y180_wf, y180_der_wf = np.array(
    drag_gaussian_pulse_waveforms(y180_amp, y180_len, y180_sigma,anharmonicity=200.01e6, alpha=drag_coef, detuning=det)
)
...
y90_wf, y90_der_wf = np.array(
    drag_gaussian_pulse_waveforms(y90_amp, y90_len, y90_sigma,anharmonicity=200.01e6, alpha=drag_coef, detuning=det)
)
...
minus_y90_wf, minus_y90_der_wf = np.array(
    drag_gaussian_pulse_waveforms(
        minus_y90_amp, minus_y90_len, minus_y90_sigma,anharmonicity=200.01e6, alpha=drag_coef, detuning=det
    )
)
...
# Resonator
resonator_IF = 6920000000.0 - 6874450000.0
resonator_LO = 6920000000.0
...
short_readout_amp = 0.1
readout_len = 2000  # 760
readout_amp = 0.1
long_readout_len = 2000
long_readout_amp = 0.1

# IQ Plane
rotation_angle = (847.163808943518 / 180) * np.pi
ge_threshold = -0.008806110502373142
...
config = {
    ...
    "controllers": {
        "con1": {
            "analog_outputs": {
                1: {"offset": -0.037607919208423685},  # q0 I
                2: {"offset": 0.06422013330843741},  # q0 Q
                3: {"offset":  -0.034331541768303324},  #
                4: {"offset": -0.10760145031978417},  #
            },
            "digital_outputs": {},
            "analog_inputs": {
